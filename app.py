...
# @app.route("/")
# def home():
#     return jsonify({"message": "Hello, Replit!"})
        ...
        try:
            ...
            for item in items:
                vid = (item["id"]["videoId"]
                       ...
                       and "videoId" in item["id"] else item.get("id"))
                if not vid:
                    continue
                title = item["snippet"]["title"]
...
def get_songs():
    ...
    try:
        ...
        if backup_data:
            ...
            return jsonify(
                {"error": "Failed to fetch songs and no cache available"}), 500
...
def get_play_url(video_id):
    ...
    try:
        ...
        content_type = "video/mp4" if format_ext == "mp4" else "application/vnd.apple.mpegurl"
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f"inline; filename={video_id}.{format_ext}"
            }
        )
